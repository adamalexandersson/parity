...
function renderParityComponent(ParityComponent $component, string $slot = ''): string
{
...
it('uses the default shell when a theme view is missing for structure components', function () {
    $html = renderParityComponent(new ShellStructureTestComponent, 'Inner content');
...
it('uses the default shell when a theme view is missing for slot-only components', function () {
    $html = renderParityComponent(new ShellSlotTestComponent, 'Heading text');
...
it('allows a theme view to override the default shell', function () {
    $html = renderParityComponent(new ShellOverrideTestComponent, 'Override content');
...
it('resolves a linkable root tag to an anchor', function () {
    $html = renderParityComponent(new ShellLinkableTestComponent);
